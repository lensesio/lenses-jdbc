...
class ViewTest : FunSpec(), ProducerSetup {
  init {
    ...
    test("VIEW test") {
      ...
      val view = "myview" + System.currentTimeMillis()
      Try { conn.createStatement().executeUpdate("DROP VIEW $view") }
      conn.createStatement().executeUpdate("CREATE VIEW $view AS SELECT tpep_pickup_datetime, VendorID FROM $topic limit 10")
      val rs = conn.createStatement().executeQuery("SELECT * FROM $view")
      ...
      shouldThrow<SQLException> {
        conn.createStatement().executeQuery("SELECT * FROM $view")
      }
    }
  }
