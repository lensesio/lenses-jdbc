...
import io.lenses.jdbc4.client.domain.InsertResponse
import io.lenses.jdbc4.client.domain.Message
import io.lenses.jdbc4.client.domain.PreparedInsertResponse
import io.lenses.jdbc4.client.domain.StreamingSelectResult
import io.lenses.jdbc4.client.domain.Table
import io.lenses.jdbc4.client.domain.Topic
...
class RestClient(private val urls: List<String>,
  ...
  private fun <T> attemptAuthenticated(reqFn: (String) -> HttpUriRequest, respFn: (HttpResponse) -> T): T {
    val reqWithTokenHeaderFn: (String) -> HttpUriRequest = {
      reqFn(it).apply {
        addHeader(io.lenses.jdbc4.Constants.LensesTokenHeader, token)
      }
    }
  ...
  private fun attemptAuthenticated(endpoint: Endpoint, uri: URI) {
    ...
    val configurator = object : ClientEndpointConfig.Configurator() {
      override fun beforeRequest(headers: MutableMap<String, MutableList<String>>) {
        headers[io.lenses.jdbc4.Constants.LensesTokenHeader] = mutableListOf(token)
      }
    }
  ...
  fun prepareStatement(sql: String): PreparedInsertResponse {
    ...
    val responseFn: (HttpResponse) -> PreparedInsertResponse = {
      ...
      io.lenses.jdbc4.JacksonSupport.fromJson(entity)
    }
  ...
  fun messages(sql: String): List<Message> {
    ...
    val responseFn: (HttpResponse) -> List<Message> = {
      io.lenses.jdbc4.JacksonSupport.fromJson(it.entity.content)
    }
  ...
  companion object RestClient {
    ...
    fun <T> jsonEntity(t: T): HttpEntity {
      val entity = StringEntity(io.lenses.jdbc4.JacksonSupport.toJson(t))
