...
class LensesDriverTest : WordSpec() {
  init {
    "LsqlDriver" should {
      "return required and optional properties for connections" {
        val props = LensesDriver().getPropertyInfo("any", Properties())
      ...
      "reject invalid url" {
        LensesDriver().acceptsURL("jdbc:qqqqq") shouldBe false
      }
      "throw return null for connect when the url is invalid" {
        LensesDriver().connect("jdbc:wibble", Properties()) shouldBe null
      }
      "throw for connection when the url is null" {
        shouldThrow<SQLException> {
          LensesDriver().connect(null, Properties())
        }
      }
