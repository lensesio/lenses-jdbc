  ...
  init {
    "LsqlDriver" should {
      "return required and optional properties for connections" {
        val props = io.lenses.jdbc4.LDriver().getPropertyInfo("any", Properties())
      ...
      "accept valid single host url" {
        io.lenses.jdbc4.LDriver().acceptsURL("jdbc:lsql:kafka:http://localhost:3030") shouldBe true
      }
      "accept valid multiple host url" {
        io.lenses.jdbc4.LDriver().acceptsURL("jdbc:lsql:kafka:http://localhost:3030,http://localhost:3031") shouldBe true
      }
      "parse parameters from url" {
        val (url, props) = io.lenses.jdbc4.LDriver().parseUrl("jdbc:lsql:kafka:http://localhost:3030,http://localhost:3031?user=admin&wibble=wobble")
      ...
      "reject invalid url" {
        io.lenses.jdbc4.LDriver().acceptsURL("jdbc:qqqqq") shouldBe false
      }
      "throw return null for connect when the url is invalid" {
        io.lenses.jdbc4.LDriver().connect("jdbc:wibble", Properties()) shouldBe null
      }
      "throw for connection when the url is null" {
        shouldThrow<SQLException> {
          io.lenses.jdbc4.LDriver().connect(null, Properties())
        }
      }
      "require each url to be http or https" {
        shouldThrow<SQLException> {
          io.lenses.jdbc4.LDriver().connect("jdbc:lsql:kafka:httpq://localhost:3030", Properties())
        }
      }
    }
  }
